  ...
  var planningHtml = '<div class="deal-sidebar-card">' +
    ...
    contactEvents.map(function(ev) {
      ...
      return '<div class="sc-row" style="flex-direction:column;align-items:flex-start;gap:2px;padding:6px 0;border-bottom:1px solid var(--border)">' +
        ...
        '<span style="background:' + (ev.color || ({tache:'#3498db',rdv:'#9b59b6',chantier:'#e67e22',livraison:'#f1c40f',devis:'#16a085',suivi:'#2d6a4f',rappel:'#e74c3c',autre:'#78909c'})[ev.type] || '#2d6a4f') + ';color:#fff;padding:1px 8px;border-radius:10px;font-size:0.7rem;margin-left:6px">' + (ev.type || '') + '</span></span>' +
      '</div>';
    }).join('')) +
    '</div></div>';
  ...
  for (var day = 1; day <= daysInMonth; day++) {
    ...
    calendarHtml += '<div class="cal-cell' + (isToday ? ' cal-today' : '') + '" data-date="' + dateKey + '">' +
      ...
      dayEvents.slice(0, 3).map(function(ev) {
        ...
        return '<div class="cal-event" style="background:' + (ev.color || ({tache:'#3498db',rdv:'#9b59b6',chantier:'#e67e22',livraison:'#f1c40f',devis:'#16a085',suivi:'#2d6a4f',rappel:'#e74c3c',autre:'#78909c'})[ev.type] || '#2d6a4f') + '">' + timeStr + ev.title + '</div>';
      }).join('') +
